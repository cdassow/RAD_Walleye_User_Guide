...
knitr::opts_chunk$set(echo = F, fig.align = 'center',cache = T)
library(sp)
library(sf)
library(tidyverse)
```
...
```{r,fig.cap="Map of Ceded Territory (grey area) in Wisconsin",fig.width=5,fig.height=4,echo=F,results='hide',fig.keep='all'}
...
1. Begin typing the county name containing the lake you're interested in and the search bar will show choices closest to what you're typing (Figure \@ref(fig:lakeSelect))
...
3. The app will automatically update it self to the information for the lake you choose
...
```{r lakeSelect, fig.cap="The red box on the left indicates where the user can enter the information to select a lake to view.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/lakeEntry.png")
```

Completion of the 3 steps above will populate the app with the relevant data for the user to begin exploring. 
Because of the volume of lake and the on-the-fly filtering that app it doing, it may occasionally be sluggish to update the lake choices when you update the county of interest or the lake choice box might take a second to appear after the app is initially opened, give it a second and it'll get there.
...
* Description or screen shot of where in the app the error occurred.
...
Several boxes are populated describing lake size and depth (Figure \@ref(fig:staticPanel)). 
...
```{r staticPanel, fig.cap="Basic Lake information that is always visible at the top of the page", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/staticPanel.png")
```

## Data Tabs

Below the 'Basic Lake Information' section is a series of tabs containing specific types of data for the selected lake, others it currently resembles, and others it is likely to resemble at mid-century(Figure \@ref(fig:lakeTabs)). 
...
```{r lakeTabs, fig.cap="Tabs housing specific kinds of information comparing the selected lake to its lake class averages now and in 2050.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/lakeTabs.png")
```
...
Here the current fishing regulations for all the major gamefish (Walleye/Sauger, Largemouth Bass, Smallmouth Bass, Muskellunge, Northern Pike, and Panfish) are displayed for the user-selected lake (Figure \@ref(fig:regs1)).
...
```{r regs1, fig.cap="Regulations for the user selected waterbody (note these are only as up to date as is available online in WDNR databases, may not reflect current regulations if there has been a recent change.)", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/regs1.png")
```

Below the table describing the current regulatory regime for the selected lake, there are a series of plots that describe the regulatory characteristics of other lakes within the same lake class as the selected lake now and at mid-century.(Figure \@ref(fig:regs2)).
...
```{r regs2, fig.cap="Summary plots and table of the regulation information for 2020 and 2050 comparison system. Click on the bars on the plots to generate the tables to the right of each plot.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/regs2.png")
```
...
Stocking histories include both the distribution of stocking densities for the major age classes (the user can again click the check boxes at the top of the panel to turn on/off certain age classes of stocked walleye), and the stocking density through time for the current and future lake classifications as well as the selected lake if the lake-specific data exists (Figure \@ref(fig:stocking1), \@ref(fig:stocking2)). 

```{r stocking1, fig.cap="Distribution of stocking densities for the selected waterbody and its comparables (number fish/per acre).", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/stocking1.png")
```

```{r stocking2, fig.cap="Stocking events through time for the user selected lake and its comparables", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/stocking2.png")
```
...
This information is presented for the use selected lake with columns for 2020 and 2050. A second table is provided for lake class averages for the current and future lake classes (Figure \@ref(fig:stocking3)).

```{r stocking3, fig.cap="Stocking survival predictions from the model presented in Lawson et al. (2022). Predicitons are presented for the 2020 and 2050 time period.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/stocking3.png")
```
...
The data table at the top of this sub-tab shows the number and age of creel surveys for the lake (Figure \@ref(fig:creel1)). 
...
The minimum, lower 25% quartile, mean, upper 85% quartile, and maximum angler CPUE are shown for each species in the selected lake and its current and future lake classes.
...
```{r creel1, fig.cap="Basic information about the age and abundance of creel surveys on the user selected waterbody", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/creel1.png")
```
...
For the most part this means the fish has been harvested, but this is not without exception (Figure \@ref(fig:creel2)). 
...
The last graphic on this tab displays the number of fish harvested per trip plotted against the number of anglers on that trip (Figure \@ref(fig:creel2)). 
...
```{r creel2, fig.cap="Creel catch lengths and angler party size and harvest information.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/creel2.png")
```
...
The top portion of this tab uses box-and-whisker plots to generally show whether the selected lake is above, below, or average compared to all the other lakes in its current and future lake class (Figure \@ref(fig:tribal1)).
...
Finally, the last figure on this tab is designed to show how specific tribes use the fishery on the selected lake. The plot here shows what proportion of the total harvest declaration is attributed to each tribe for the lake (Figure \@ref(fig:tribal2)). Often only one tribe will harvest a specific lake, but there are some lakes that are used by multiple tribes. The purpose of this plot is to get the user thinking about how changes to the ogaa and maazhi-ginoozhe fisheries for the lake, whether they are changes in co-management or environmental change, will effect some tribes more than others.

```{r tribal1, fig.cap="Distribution of safe harvest and declaration data for ogaa and Maashi-ginoozhe.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/tribal1.png")
```

```{r tribal2, fig.cap="Use of the selected lake by the Ojibwe tribes in the CTWI", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/tribal2.png")
```
...
Catch-per-mile distributions are shown for YOY, or age-0, as well as age-1 walleye (Figure \@ref(fig:yoyElectrofishing)). 
...
```{r yoyElectrofishing, fig.cap="Distribution and chronological presentations of YOY walleye electrofishing catch rates.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/yoyElectrofishing.png")
```
...
Using the species specified by the user in panel where the lake was selected, the app pulls all the relevant CPE data from standardized electrofishing surveys and fyke net surveys (Figure \@ref(fig:cpes)).
These data are separated by gear type.
Similar to the YOY walleye data, the distribution of CPE values for the user-selected lake and its current and future lake class are shown for each species selected by the user for fyke netting and electrofishing gear types. 


```{r cpes, fig.cap="Sampling of the various catch-per-effort metrics available in the app for walleye and other desired gamefish species for fyke net and electrofishing gear types", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/cpes.png")
```
...
First the plot through time of the raw number of fish estimated to be in the stock with the 95% confidence interval is shown for just the user-selected lake (Figure \@ref(fig:wlyPE)). 
Alongside that is the population size expressed in terms of density (number per acre) so that comparisons can be made between the population density in the user-selected lake through time and the temporal trends in density for its current and future lake class (Figure \@ref(fig:wlyPE)). 
Below that is a table displaying the details of any population estimate available for the user-selected lake.

```{r wlyPE, fig.cap="Walleye population estimate information presented in the 'Population Metrics' tab.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/wlyPE.png")
```
...
The first figure shows length-at-age for species selected by the user across the user-selected lake and its current and future lake class (Figure \@ref(fig:lengthAge1)). 
...
```{r lengthAge1, fig.cap="Length at age information for the desired gamefish species, lengths are presented as averages (+/- 1 standard deviation) for all aged fish of a given age. In some cases no standard deviation can be calculated if only one fish of a given age has been recorded to date. In other cases standard deviations are so small that the vertical bars do not extend beyond the boundaries of the circle noting the average..", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/lengthAge1.png")
```


The data table shown here shows the number of fish aged and by what structure for each species of interest and sex. 
Similarly, below this data table is the same information grouped across sexes but shown though time to get an idea of how much and how recent the aging sampling has been for the selected lake (Figure \@ref(fig:lengthAge2)).

```{r lengthAge2, fig.cap="Chronological distribution of ageing samples taken for the selected lake.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/lengthAge2.png")
```
...
This tab is split into two sub-tabs, 'Substitutable Walleye Options' and 'Travel Costs'. The first tab is designed to present the user with alternative walleye lakes within a few different driving radii (Figure \@ref(fig:substituteOptions)). 
...
```{r substituteOptions, fig.cap="number of lakes withing 30, 45, and 60 mi driving radii of the user selected lake. Lake class is noted on the x axis. These plots can be clicked by the user to pull up the identity of the lakes in each bar of the plot.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/substituteOptions.png")
```
...
The user may enter any city within the U.S. though we expect the user to mainly be interested in anglers traveling from different places within Wisconsin and the bordering states (Figure \@ref(fig:travelDist)). 
In addition to specifying a starting location for an angler, the user also specifies what lake classes to consider and how many lakes of each lake class to consider (Figure \@ref(fig:travelDist)). 
...
This information is presented for the years 2020 and 2050 because as lakes change over time, the cost of traveling to an equivalent walleye fishing opportunity will change as more lakes become unsuitable for walleye, something captured by the changing lake classifications, anglers may have to travel farther and farther to reach the remaining walleye fisheries around the state (Figure \@ref(fig:travelDist)).

```{r travelDist, fig.cap="The location of the fisher origin desired by the user is entered in the red box shown here. Also in the red box, the user may enter the number and type of alternative fish options they want to consider for fishers coming from the selected point of origin.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/travelDist.png")
```
...
First, the user can use the AIS smart prevention tool to see what common non-native species might be able to establish themselves in Wisconsin lakes and streams to see what potential risks exist for the lake they're interested in (Figure \@ref(fig:invasives)). 
Immediately below this, the confirmed non-native species present in the selected lake are shown.
In order to provide some context, the percentage of other lakes in that same lake class with each of the common non-native species is also shown for the 2020 and 2050 lake classes of the user-selected lake (Figure \@ref(fig:invasives)).
...
```{r invasives, fig.cap="AIS SMART prevention tool is highlighted by the red box. This link to an external tool and help the user understand the vulnerability of their system to invasion of common non-native aquatic species in the state. Sumary information on the non-native species present is also provided on the page in the text and tables below the red box.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/invasives.png")
```
...
*this tab is the least developed, still seeking input from users on what they would like to see here* 
...
Most importantly, the goal of the this tab is to remind the user that any decision taken with respect to walleye management in the selected lake will fall into one of the 3 RAD categories described in this tab (Figure \@ref(fig:downloadButton)). 
...
An option exists for the app user to download a summary report for the selected lake that provides some figures and text about how the selected lake compares to other similar lakes now and in the future (Figure \@ref(fig:downloadButton)). 
This information is largely presented as percentiles instead of raw numbers.
The thinking here is that this is easier to digest for most potential readers who may not have a fisheries management background. The content of this report still needs improving and suggestions from app users are especially welcome here. 
Feel free to reach out to Colin with those suggestions.

```{r downloadButton, fig.cap="Summary of the walleye recruitment and population size information, as well as the RAD context, is provided here. The red arrow points out where the button is to download a report to be shared with others.", out.height='85%',out.width='85%'}
knitr::include_graphics("C:/Users/dassocju/OneDrive - State of Wisconsin/replacementCosts/rcApp/userGuideScreenShots/downloadButton.png")
```
